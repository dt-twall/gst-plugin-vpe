...
#define __PARSER_DECODER_VPE_BIN_TEMPLATE(type, parser_name, decoder_name)    \
  \
typedef struct {               \
  GstBin parent;               \
} type;                        \
...
{                                                                            \
  ...
  par = gst_element_factory_make (#parser_name, "parser");                   \
  if (!par) {                                                                \
    GST_ERROR("Cannot create (" #parser_name  ") element");                  \
    gst_object_unref(GST_OBJECT(vpe));                                       \
    gst_object_unref(GST_OBJECT(dec));                                       \
    return;                                                                  \
  }                                                                          \
  gst_bin_add_many(GST_BIN(self), dec, vpe, par, NULL);                      \
  gst_element_link_many(par, dec, vpe, NULL);                                \
  tmp = gst_element_get_static_pad(par, "sink");                             \
  if (tmp) {                                                                 \
    sinkpad = gst_ghost_pad_new("sink", tmp);                                \
  }                                                                          \
  tmp = gst_element_get_static_pad(vpe, "src");                              \
...
__PARSER_DECODER_VPE_BIN_TEMPLATE(GstDucatiJpegdecVpe, jpegparse, ducatijpegdec);
